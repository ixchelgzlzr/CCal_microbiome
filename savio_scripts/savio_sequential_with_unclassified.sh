...
OUT_DIR="output/metaphlan_out_with_unclassified"
...
while read -r sample; do
  ...
  PROFILE="output/metaphlan_out_with_unclassified/${sample}_metaphlan.txt"
  MAPOUT="output/metaphlan_out_with_unclassified/${sample}_map.bowtie2.bz2"
  LOG="output/metaphlan_out_with_unclassified/${sample}_metaphlan.log"
  ...
  metaphlan "${R1},${R2}" --input_type fastq --db_dir "$DB_DIR" -x mpa_vJan25_CHOCOPhlAnSGB_202503 --offline --verbose --mapout "$MAPOUT" --nproc 20 > "$PROFILE" 2> "$LOG"
